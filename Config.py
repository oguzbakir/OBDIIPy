...
class Config(Visual.Visual):
    def __init__(self, ThisSurface, Name, Text):
        ...
        Visual.Visual.__init__(self, ThisSurface, Name, Visual.PRESS_NONE, 0, 0, Button.Visual.BUTTON_HEIGHT,
                               Button.Visual.BUTTON_HEIGHT, Text)
    ...
    def Display(self, ThisSurface, xOffset=0, yOffset=0):
        ...
        pygame.draw.rect(ThisSurface, self.ColourFill, (xOffset + self.xPos, yOffset + self.yPos, self.xLen, self.yLen),
                         0)
        pygame.draw.rect(ThisSurface, self.ColourBorder,
                         (xOffset + self.xPos, yOffset + self.yPos, self.xLen, self.yLen), 4)
        ...
        ThisSurface.blit(RenderText[0],
                         (self.xPos + xOffset + 2 * Visual.X_MARGIN + Visual.BUTTON_HEIGHT, self.yPos + yOffset + 10))
        ThisText = ConfigValues["FontName"]
        TextHeight = Visual.Fonts["LargeFont"].get_rect(ThisText)[3]
        RenderText = Visual.Fonts["LargeFont"].render(ThisText, self.ColourValueText)
        ThisSurface.blit(RenderText[0], (self.xPos + xOffset + 2 * Visual.X_MARGIN + Visual.BUTTON_HEIGHT,
                                         self.yPos + yOffset + 10 + TextHeight + Visual.Y_MARGIN))
        ...
        RenderText = Visual.Fonts["LargeFont"].render(ThisText, self.ColourText)
        ThisSurface.blit(RenderText[0],
                         (self.xPos + xOffset + 2 * Visual.X_MARGIN + Visual.BUTTON_HEIGHT, self.yPos + yOffset + 85))
        ThisText = ConfigValues["SerialPort"]
        TextHeight = Visual.Fonts["LargeFont"].get_rect(ThisText)[3]
        RenderText = Visual.Fonts["LargeFont"].render(ThisText, self.ColourValueText)
        ThisSurface.blit(RenderText[0], (self.xPos + xOffset + 2 * Visual.X_MARGIN + Visual.BUTTON_HEIGHT,
                                         self.yPos + yOffset + 85 + TextHeight + Visual.Y_MARGIN))
        ...
        RenderText = Visual.Fonts["LargeFont"].render(ThisText, self.ColourText)
        ThisSurface.blit(RenderText[0],
                         (self.xPos + xOffset + 2 * Visual.X_MARGIN + Visual.BUTTON_HEIGHT, self.yPos + yOffset + 160))
        ThisText = ConfigValues["Vehicle"]
        TextHeight = Visual.Fonts["LargeFont"].get_rect(ThisText)[3]
        RenderText = Visual.Fonts["LargeFont"].render(ThisText, self.ColourValueText)
        ThisSurface.blit(RenderText[0], (self.xPos + xOffset + 2 * Visual.X_MARGIN + Visual.BUTTON_HEIGHT,
                                         self.yPos + yOffset + 160 + TextHeight + Visual.Y_MARGIN))
        ...
        RenderText = Visual.Fonts["LargeFont"].render(ThisText, self.ColourText)
        ThisSurface.blit(RenderText[0],
                         (self.xPos + xOffset + 2 * Visual.X_MARGIN + Visual.BUTTON_HEIGHT, self.yPos + yOffset + 235))
        ThisText = ConfigValues["Debug"]
        TextHeight = Visual.Fonts["LargeFont"].get_rect(ThisText)[3]
        RenderText = Visual.Fonts["LargeFont"].render(ThisText, self.ColourValueText)
        ThisSurface.blit(RenderText[0], (self.xPos + xOffset + 2 * Visual.X_MARGIN + Visual.BUTTON_HEIGHT,
                                         self.yPos + yOffset + 235 + TextHeight + Visual.Y_MARGIN))
