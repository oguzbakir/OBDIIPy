...
class Plot(Visual.Visual):
    ...
    def Display(self, ThisSurface):
        ...
        for yOffset in range(0, yAxisScale, yAxisStep):
            pygame.draw.line(ThisSurface, self.ColourGrey,
                             (self.xPos + Visual.X_MARGIN, self.yPos + self.yLen - yOffset),
                             (self.xPos + self.xLen - 2 * Visual.X_MARGIN, self.yPos + self.yLen - yOffset), 1)
        # Display data scale.
        ...
        for Index in self.xAxisLabels:
            ...
            pygame.draw.line(ThisSurface, self.ColourGrey, (TextXPos, self.yPos + Visual.Y_MARGIN),
                             (TextXPos, self.yPos + self.yLen - 2 * Visual.Y_MARGIN), 1)
        ...
        for Index in range(PLOT_COUNT):
            ...
            if len(TextLabels) > ELM327.FIELD_PID_FORMAT_1 and TextLabels[ELM327.FIELD_PID_FORMAT_1].find("f}") > -1:
                ThisText += " " + TextLabels[ELM327.FIELD_PID_FORMAT_1].format(
                    self.PlotPoints[Index][self.PlotIndex[Index] - 1])
                ...
                # Plot series scale.
                yScale = (self.yLen - 2 * Visual.Y_MARGIN) / (
                            self.PlotAttrib[Index]["ValueMax"] - self.PlotAttrib[Index]["ValueMin"])
                xStep = (self.xLen - 2 * Visual.X_MARGIN) / PLOT_POINTS
                # Display Y axis scale values.
                for yOffset in range(0, yAxisScale - yAxisStep, yAxisStep):
                    ThisText = TextLabels[ELM327.FIELD_PID_FORMAT_1].format(
                        yOffset / yScale + self.PlotAttrib[Index]["ValueMin"])
                ...
                for PlotIndex in range(self.PlotIndex[Index] - 1):
                    ...
                    PlotY1 = self.yPos + self.yLen - Visual.Y_MARGIN - yScale * (
                                self.PlotPoints[Index][PlotIndex] - self.PlotAttrib[Index]["ValueMin"])
                    PlotX2 = Visual.X_MARGIN + xOffset + xStep
                    PlotY2 = self.yPos + self.yLen - Visual.Y_MARGIN - yScale * (
                                self.PlotPoints[Index][PlotIndex + 1] - self.PlotAttrib[Index]["ValueMin"])
                    pygame.draw.line(ThisSurface, self.PlotAttrib[Index]["Colour"], (PlotX1, PlotY1), (PlotX2, PlotY2),
                                     PLOT_WIDTH)
                    xOffset += xStep
