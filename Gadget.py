...
# /* vertical bar, horizontal bar, info...                                    */
# /***************************************************************************/
...
class Gadget(Visual.Visual):
    ...
    def IsEvent(self, EventType, xPos, yPos, PointerButton, xOffset=0, yOffset=0):
        ...
        if Result != False:
            ...
            Result["GADGET"] = self.Name
            # Tell caller the name of the button on the gague that was touched.
            Result["BUTTON"] = ""
            for ThisButton in self.Buttons:
                ButtonResult = self.Buttons[ThisButton].IsEvent(EventType, xPos, yPos, PointerButton, self.xPos,
                                                                self.yPos)
    ...
    def SetStyle(self, Style):
        ...
        # Apply currently selected gadget style.
        ...
        # Arrange buttons so they don't clash in any gadget type.
    ...
    # /*********************************************/
    # /* Draw this gadget on the provided surface. */
    # /*********************************************/
    def Display(self, ThisSurface, xOffset=0, yOffset=0):
        ...
        if self.Style == STYLE_GAUGE:
            ...
            pygame.draw.arc(self.ThisSurface, self.BarHighColour, (
            4 * Visual.X_MARGIN + self.xPos, 4 * Visual.Y_MARGIN + self.yPos, self.xLen - 8 * Visual.X_MARGIN,
            self.xLen - 8 * Visual.Y_MARGIN), -2 * math.pi - 0.6 * math.pi / 2,
                            2 * math.pi * -PointerHighRatio - math.pi / 2, 2 * Visual.X_MARGIN)
            for Angle in range(0, 100, 10):
                ...
                if Angle == 0:
                    ...
                    TickWidth = 5
                pygame.draw.line(self.ThisSurface, self.ColourBlack, (OriginX + AngleX1, OriginY + AngleY1),
                                 (OriginX + AngleX2, OriginY + AngleY2), TickWidth)
                ...
                ThisSurface.blit(RenderText[0], (
                OriginX + AngleX3 - Visual.Fonts["NormalFont"].get_rect(ThisText)[2] / 2, OriginY + AngleY3))

            # Draw gague values.
            TextLabels = self.PidDescription.split("|")

            DisplayText = self.LayoutText(TextLabels[ELM327.FIELD_PID_DESCRIPTION], 2, self.xLen - 4 * Visual.X_MARGIN,
                                          Visual.Fonts["LargeFont"])
            DisplayTextOffset = 0
            for DisplayTextLine in DisplayText.split('\n'):
                ...
                TextYPos = DisplayTextOffset + self.yPos + self.yLen - Visual.Y_MARGIN - 2 * (
                            TextHeight + Visual.Y_MARGIN)
            ...
            pygame.draw.line(self.ThisSurface, self.PointerColour, (OriginX, OriginY),
                             (OriginX + PointerX, OriginY + PointerY), self.PointerWidth)
        ...
        # Draw the gague as a vertical bar gague.
            ...
            # Draw gague background.
            pygame.draw.rect(ThisSurface, self.FillColour, (
            Visual.X_MARGIN + self.xPos, Visual.Y_MARGIN + self.yPos, self.xLen - 2 * Visual.X_MARGIN,
            self.yLen - 2 * Visual.Y_MARGIN), 0)
            # Draw gague pointer.
            PointerYPos = Visual.Y_MARGIN + self.yPos + int(
                (self.yLen - 2 * Visual.Y_MARGIN) - (self.yLen - 2 * Visual.Y_MARGIN) * PointerRatio)
            PointerYHighPos = Visual.Y_MARGIN + self.yPos + int(
                (self.yLen - 2 * Visual.Y_MARGIN) - (self.yLen - 2 * Visual.Y_MARGIN) * PointerHighRatio)
            pygame.draw.rect(ThisSurface, self.BarColour, (
            Visual.X_MARGIN + self.xPos, PointerYPos, self.xLen - 2 * Visual.X_MARGIN,
            self.yPos + self.yLen - Visual.Y_MARGIN - PointerYPos), 0)
            if ThisValue >= self.ValueHigh:
                pygame.draw.rect(ThisSurface, self.BarHighColour, (
                Visual.X_MARGIN + self.xPos, PointerYPos, self.xLen - 2 * Visual.X_MARGIN,
                PointerYHighPos - PointerYPos), 0)
            pygame.draw.line(self.ThisSurface, self.PointerColour, (Visual.X_MARGIN + self.xPos, PointerYPos),
                             (self.xPos + self.xLen - Visual.X_MARGIN - self.PointerWidth / 2, PointerYPos),
                             self.PointerWidth)

            # Draw gague values.
            TextLabels = self.PidDescription.split("|")

            DisplayText = self.LayoutText(TextLabels[ELM327.FIELD_PID_DESCRIPTION], 2, self.yLen - 4 * Visual.Y_MARGIN,
                                          Visual.Fonts["LargeFont"])
            DisplayTextOffset = 0
            for DisplayTextLine in DisplayText.split('\n'):
                ThisText = DisplayTextLine
        ...
        # Draw the gague as a horizontal bar gague.
            ...
            pygame.draw.rect(ThisSurface, self.BarColour, (
            Visual.X_MARGIN + self.xPos, Visual.Y_MARGIN + self.yPos, PointerXPos - Visual.X_MARGIN - self.xPos,
            self.yLen - 2 * Visual.Y_MARGIN), 0)
            if ThisValue >= self.ValueHigh:
                pygame.draw.rect(ThisSurface, self.BarHighColour, (
                PointerXPos, Visual.Y_MARGIN + self.yPos, PointerHighXPos - PointerXPos,
                self.yLen - 2 * Visual.Y_MARGIN), 0)
            pygame.draw.line(self.ThisSurface, self.PointerColour, (PointerXPos, self.yPos + Visual.Y_MARGIN),
                             (PointerXPos, self.yPos + self.yLen - Visual.Y_MARGIN - self.PointerWidth / 2),
                             self.PointerWidth)
        ...
        # Display all buttons on the gadget.
        for ThisButton in self.Buttons:
