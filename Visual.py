...
class Visual:
    def __init__(self, ThisSurface, Name, PressType, xPos, yPos, xLen, yLen, Text, Align=ALIGN_TEXT_CENTER,
                 DownText=""):
    ...
    def Display(self, ThisSurface, xOffset=0, yOffset=0):
        if self.Visible == True:
            if self.Text[:6] == "IMAGE:" and self.Down == False:
                ...
                ThisSurface.blit(ScaledPng, (xOffset + self.xPos + (self.xLen - BUTTON_HEIGHT + 7) / 2,
                                             yOffset + self.yPos + (self.yLen - BUTTON_HEIGHT + 7) / 2))
                ...
                for TextLine in TextLines:
                    ...
                    if self.Align == ALIGN_TEXT_LEFT:
                        pygame.draw.line(self.ThisSurface, self.ColourGrey,
                                         (X_MARGIN + self.xPos + xOffset, self.yPos + yOffset + Y_MARGIN / 2), (
                                         self.xPos + self.xLen - 2 * X_MARGIN - xOffset,
                                         self.yPos + yOffset + Y_MARGIN / 2), 1)
                        ...
                        if self.MouseXPos >= X_MARGIN + self.xPos + xOffset and self.MouseXPos <= TextXPos + self.xLen - 3 * X_MARGIN - xOffset and self.MouseYPos >= TextYPos and self.MouseYPos <= TextYPos + FontHeight:
                            ...
                            pygame.draw.rect(self.ThisSurface, self.ColourFillDown, (
                            X_MARGIN + self.xPos + xOffset, self.yPos + yOffset + Y_MARGIN / 2,
                            self.xLen - 3 * X_MARGIN - xOffset, FontGap), 0)
